...
export default function CarrosselProjetos() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-7xl pb-16 overflow-hidden">
      {/* Cards */}
        ...
        {projetos.map((projeto, index) => {
          ...
          return (
            <article
              key={projeto.id}
              className={clsx(
                "relative w-[85%] sm:w-[300px] min-h-[500px] sm:min-h-[600px] rounded-3xl p-[1px] transition-all duration-300",
                isActive && "sm:scale-100 sm:opacity-100 sm:z-10",
                !isActive && "sm:scale-90 sm:opacity-40 sm:-z-0",
                "cursor-pointer sm:cursor-default bg-gradient-to-b from-white/15 via-white/8 to-white/5 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] sm:hover:-translate-y-1"
              )}
            >
                ...
                <div className="flex flex-1 flex-col p-4">
                  ...
                  <div className="mb-4 flex flex-wrap gap-2">
                    {projeto.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="rounded-full bg-white/15 px-2 py-1 text-[10px] sm:text-xs text-white ring-1 ring-white/10"
                      >
                        {tag}
                      </span>
                    ))}
      ...
      <button
        onClick={handlePrev}
        className="hidden sm:flex  absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/10 p-2 ring-1 ring-white/15 backdrop-blur hover:bg-white/15"
        aria-label="Anterior"
      >
      ...
      <button
        onClick={handleNext}
        className="hidden sm:flex absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/10 p-2 ring-1 ring-white/15 backdrop-blur hover:bg-white/15"
        aria-label="Próximo"
      >
        <svg width="48" height="48" viewBox="0 0 125 125" fill="none" xmlns="http://www.w3.org/2000/svg">
